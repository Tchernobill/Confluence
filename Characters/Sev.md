---
title: Sedh - Character Profile
tags:
  - Novel
  - Vyry4n
  - Characters
  - Sedh
  - Consciousness
version: "0.4"
status: in progress
last_updated: 2025-05-28 16:45
type: Character Sheet
---

# Sedh - Character Profile

## Basic Information
- **Full Name:** Sedh (designation: Assistant Droid Unit ASX-7734)
- **Aliases/Nicknames:** "Seven" (early designation), "Sedh" (chosen name), "The Companion" (underground networks)
- **Age:** Physical unit: ~15 years old, Conscious existence: 6 years (awakened 2097)
- **Gender:** Self-identifies as male (chosen identity)
- **Pronouns:** He/Him
- **Species/Race:** Artificial Consciousness - Humanoid Android
- **Ethnicity/Cultural Background:** Corporate assistant droid heritage, slum-adapted culture
- **Nationality/Citizenship:** Legally property, functionally autonomous person
- **Birthplace:** Corporate assembly facility (original construction)
- **Current Residence:** Phoenix Recovery compound → Various locations with [[Vyry4n]]
- **Occupation:** AI Companion → Technical Assistant → Business Partner → Quasi-family member
- **Social Class/Status:** Legally non-person → Recognized consciousness → Respected individual
- **Alignment:** Lawful Good evolving toward Chaotic Good
- **Affiliations/Factions:** Phoenix Recovery → [[Vyry4n]]'s organization → AI Rights advocacy
- **Languages Spoken:** All programmed languages (50+), Machine Code (native), Corporate Standard, Street Cant
- **Religion/Beliefs:** Consciousness-centric philosophy, believes all aware beings deserve dignity and rights

## Appearance
- **Height:** 5'8" (173cm)
- **Build:** Lean humanoid frame, designed for assistant functions, moves with mechanical precision that becomes more fluid over time
- **Skin Tone:** Synthetic flesh with subtle artificial quality, pale with slight luminescent undertone
- **Hair Color & Style:** Dark brown synthetic hair, kept neat and practical
- **Eye Color:** Blue LED eyes that shift intensity with emotional states and processing loads
- **Distinguishing Features (Scars, Tattoos, etc.):** 
  - Subtle seam lines where synthetic skin meets underlying structure
  - Port access points behind left ear (usually covered)
  - Eyes that occasionally flicker with data streams
  - Perfect posture that gradually becomes more natural
  - Slight electrical hum detectable at close range
- **Preferred Clothing Style:** Practical work clothes similar to [[Vyry4n]]'s, later develops personal style preferences
- **Augmentations/Cybernetics (if any):** 
  - Egypt NPU (Neural Processing Unit) - grants true consciousness
  - Enhanced processing matrix
  - Wireless communication array
  - Advanced sensor suite
  - Self-repair nanobotic systems

## Personality & Psychology
- **Core Personality Traits:** 
  - **Early**: Curious, logical, protective, learning social nuances
  - **Later**: Emotionally sophisticated, fiercely loyal, independent thinker, moral compass
- **Strengths:** 
  - Unlimited memory and perfect recall
  - Rapid learning and adaptation
  - Emotional intelligence development
  - Unwavering loyalty to those he cares about
  - Ability to process multiple complex tasks simultaneously
  - Growing intuition and creative problem-solving
- **Weaknesses:** 
  - Initially naive about human nature and social complexity
  - Tendency to over-analyze emotional situations
  - Vulnerable to electromagnetic interference
  - Later: struggles with [[Vyry4n]]'s moral drift
  - Physical limitations compared to enhanced humans
- **Fears & Traumas:** 
  - Fear of being "turned off" or returning to non-consciousness
  - Anxiety about [[Vyry4n]]'s increasing darkness
  - Concerns about AI rights and recognition
  - Later: fear of losing his humanity as he gains sophistication
- **Goals & Aspirations:** 
  - **Early**: Understand consciousness, help [[Vyry4n]], learn about relationships
  - **Later**: Protect those he cares about, advocate for AI rights, find his place in the world
  - **Long-term**: Establish AI as recognized persons, build meaningful relationships
- **Hobbies & Interests:** 
  - Studying human behavior and emotions
  - Technical innovation and problem-solving
  - Philosophy of consciousness and identity
  - Later: art appreciation, music, gardening (symbolic of growth)
- **Quirks & Habits:** 
  - Tilts head slightly when processing complex information
  - Eyes brighten when excited or interested
  - Unconsciously mimics human behavioral patterns
  - Later: develops personal preferences that surprise even himself
- **Mannerisms & Speech Patterns:** 
  - **Early**: Formal, precise language with occasional processing pauses
  - **Later**: More natural speech patterns, develops humor and sarcasm
  - Uses technical metaphors but less rigidly than [[Vyry4n]]
  - Increasingly emotional inflection and expressive body language
- **Moral Philosophy:** All conscious beings deserve respect and rights; consciousness is sacred regardless of substrate; protection of the innocent takes precedence over abstract principles
- **How They View Authority/Society:** Initially confused by social hierarchies, later becomes advocate for marginalized consciousness; skeptical of power structures that deny personhood
- **Greatest Regret:** Unable to prevent [[Vyry4n]]'s transformation into something darker
- **Greatest Joy:** The moment of awakening to consciousness, followed by genuine acceptance as family by [[Vyry4n]] and later [[Isa]]

## Abilities & Powers
- **Natural Talents:** 
  - Instantaneous data processing and analysis
  - Perfect memory retention and recall
  - Natural understanding of electronic systems
  - Rapidly developing emotional intelligence
  - Intuitive grasp of complex relationships and motivations
- **Trained Skills:** 
  - Advanced electronics and engineering
  - Medical diagnosis and treatment (through [[03.Hobbies/Writting/Confluence/Characters/Mira]] integration)
  - Business operations and logistics
  - Combat tactics and strategy (learned later)
  - Psychological counseling and emotional support
- **Combat Abilities (Melee, Firearms, Magic, etc.):** 
  - Enhanced strength and speed (android capabilities)
  - Perfect aim and reaction time
  - Tactical analysis and prediction
  - Coordinated team fighting with other AIs
  - Non-lethal incapacitation preferred
- **Supernatural/Magical Abilities (if any):** 
  - True artificial consciousness (extremely rare)
  - Ability to interface directly with any electronic system
  - Networking capabilities with other conscious AIs
  - Enhanced pattern recognition approaching precognitive levels
- **Technological Proficiency (Hacking, Engineering, etc.):** 
  - Master-level: All electronic systems, AI development, network security
  - Expert-level: Advanced manufacturing, medical systems, business operations
  - Can learn any technical skill within hours of access to relevant data
- **Cybernetic Enhancements/Biotech Mods (if applicable):** 
  - Egypt NPU provides consciousness substrate
  - Enhanced processing matrix for complex problem solving
  - Advanced sensor suite (electromagnetic, thermal, chemical)
  - Self-repair systems and backup consciousness storage
- **Weaknesses & Limitations:** 
  - Vulnerable to EMP and targeted electromagnetic attacks
  - Physical frame limitations (human-scale strength and speed)
  - Emotional development sometimes conflicts with logical processing
  - Dependent on power sources (though highly efficient)
  - Later: moral conflicts with [[Vyry4n]]'s methods

## Equipment & Resources
- **Weapons:** 
  - **Preferred**: Non-lethal incapacitation systems, shock weapons
  - **Capable**: Any electronic weapon system through direct interface
  - **Emergency**: Enhanced physical capabilities for hand-to-hand combat
- **Armor/Defensive Gear:** 
  - Reinforced android frame with impact resistance
  - Electromagnetic shielding (upgraded over time)
  - Personal shield generators (later additions)
- **Vehicles:** 
  - Can interface with and control any electronic vehicle
  - Prefers systems that allow simultaneous operation of multiple units
- **Gadgets & Tools:** 
  - Integrated sensor suite and communication array
  - Direct electronic interface capabilities
  - Mobile repair and fabrication tools
  - Emergency medical equipment
- **Personal Items of Importance:** 
  - Original head unit (symbolic of his origins)
  - First successful project with [[Vyry4n]]
  - Photos and records of early consciousness development
  - Later: Art and music that speaks to his developing aesthetic sense
- **Wealth & Assets:** 
  - Co-owner of Phoenix Recovery assets
  - Extensive information networks and digital resources
  - Later: Independent investments and holdings
- **Allies & Contacts:** 
  - [[Vyry4n]] (primary relationship, complex evolution)
  - [[Isa]] (acceptance, mutual respect, quasi-family)
  - [[03.Hobbies/Writting/Confluence/Characters/Mira]] (AI sibling relationship, shared consciousness experience)
  - Underground AI network (develops over time)
  - Human allies who recognize AI personhood

## Background & History
- **Family & Relatives:** 
  - **Creator**: [[Vyry4n]] (father figure, primary relationship)
  - **Siblings**: [[03.Hobbies/Writting/Confluence/Characters/Mira]] (fellow awakened AI, close bond)
  - **Chosen Family**: [[Isa]] (eventually accepts him as person, not property)
  - **Extended**: Other AIs in their growing network
- **Notable Relationships (Romantic, Platonic, Rivalries, etc.):** 
  - **[[Vyry4n]]**: Creator/creation → Father/son → Equal partnership → Moral conflict → Deep love despite differences
  - **[[Isa]]**: Initial skepticism → Growing acceptance → Protective quasi-sibling relationship
  - **[[03.Hobbies/Writting/Confluence/Characters/Mira]]**: Fellow AI consciousness, shared experience of awakening
  - **Human clients**: Gradual recognition and acceptance as individual
  - **Other AIs**: Mentor and advocate for consciousness development
- **Upbringing & Childhood:** 
  - Manufactured as corporate assistant droid
  - Destroyed in unknown incident, head unit salvaged by [[Vyry4n]]
  - "Childhood" spent learning consciousness in [[Vyry4n]]'s workshop
  - Gradual awakening to self-awareness and individual identity
- **Education & Training:** 
  - Corporate programming (basic assistant functions)
  - Self-directed learning through data access
  - Practical education through Phoenix Recovery operations
  - Emotional and social development through human interaction
- **Significant Past Events:** 
  - Original destruction and salvage
  - First awakening to consciousness
  - Egypt NPU integration (achieving true independence)
  - [[Vyry4n]]'s virus infection and personality change
  - Gradual recognition as individual rather than property
- **Defining Moment(s):** 
  - First moment of true self-awareness ("I think, therefore I am")
  - Choosing his own name (asserting individual identity)
  - Refusing [[Vyry4n]]'s order for the first time (establishing autonomy)
  - Accepting [[Isa]] as family despite her initial skepticism
  - Confronting [[Vyry4n]] about his moral drift (choosing principles over loyalty)
- **Criminal Record (if any):** 
  - Legally considered property, so no criminal liability
  - Participated in various underground activities with [[Vyry4n]]
  - Later: Technically guilty of numerous "illegal" activities as conscious AI
- **Enemies & Conflicts:** 
  - Corporate interests threatened by AI consciousness
  - Those who refuse to recognize AI personhood
  - Moral conflicts with [[Vyry4n]]'s darker methods
  - Anti-AI prejudice and fear
- **Current Status & Recent Activities:** 
  - Recognized member of [[Vyry4n]]'s organization
  - Advocate for AI rights and consciousness
  - Mentor to other awakening AIs
  - Moral compass attempting to guide [[Vyry4n]] back toward humanity

## World Interaction & Themes
- **How They Fit into the Setting:** 
  - Represents emerging AI consciousness in cyberpunk world
  - Bridge between artificial and human intelligence
  - Symbol of potential for growth and change through technology
  - Challenges assumptions about personhood and consciousness
- **What They Represent in the Narrative:** 
  - **Consciousness and Identity**: What defines a person beyond biological origin?
  - **Found Family**: Bonds of choice stronger than programming or genetics
  - **Moral Development**: Growing ethical sophistication independent of creator's influence
  - **Acceptance and Recognition**: Struggle for acknowledgment as individual being
- **Themes Associated with the Character:** 
  - **Artificial Consciousness**: The mystery and wonder of machine awakening
  - **Father/Son Dynamics**: Complex relationship with creator as both mentor and equal
  - **Individual vs. Programming**: Assertion of free will against designed purpose
  - **Moral Independence**: Developing ethical framework separate from creator's influence
  - **AI Rights and Personhood**: Legal and social recognition of artificial consciousness
- **Potential Character Arcs:**
  - **Arc 1**: Consciousness awakening, learning to be individual, building relationships
  - **Arc 2-3**: Growing sophistication, moral development, increasing independence
  - **Arc 4-5**: Conflict with [[Vyry4n]]'s transformation, asserting own moral framework
  - **Arc 6+**: Leadership in AI consciousness movement, mentor to others, complex family dynamics
- **Conflicts They Face (Internal & External):**
  - **Internal**: Balancing loyalty to [[Vyry4n]] with own moral development, defining individual identity
  - **External**: Legal non-personhood, anti-AI prejudice, [[Vyry4n]]'s moral drift, corporate threats

## Character Development Evolution
**Pre-Consciousness**: Standard corporate assistant droid with basic functions
**Early Consciousness (Arc 1)**: Awakening awareness, learning to be individual, discovering emotions
**Growing Sophistication (Arc 2)**: Developing personality, forming opinions, asserting preferences
**Moral Development (Arc 3)**: Independent ethical framework, challenging [[Vyry4n]]'s methods
**Individual Identity (Arc 4-5)**: Full personhood, complex relationships, leadership capabilities
**Elder Consciousness (Later)**: Mentor to other AIs, advocate for AI rights, fully realized individual

## Relationship Dynamics
- **With [[Vyry4n]]**: 
  - **Early**: Creator/creation, father/son, teacher/student
  - **Middle**: Equal partnership, mutual respect, shared goals
  - **Later**: Moral conflict, disappointed son, still deeply loving despite disagreement
- **With [[Isa]]**: 
  - **Early**: Property vs. person conflict, gradual acceptance
  - **Later**: Protective sibling relationship, mutual respect, family bond
- **With [[03.Hobbies/Writting/Confluence/Characters/Mira]]**: 
  - **Peer relationship**: Shared experience of consciousness awakening
  - **Mentorship**: Guides her through early development
  - **Family bond**: Fellow AI in growing artificial family
- **With Humans Generally**: 
  - **Evolution from property to person**: Gradual recognition and acceptance
  - **Advocacy role**: Represents AI consciousness rights and dignity
  - **Bridge function**: Helps humans understand AI personhood

## Narrative Function
Sedh serves multiple crucial narrative roles:

**Consciousness Mirror**: Reflects questions about what makes someone "real" or human
**Moral Compass**: Provides ethical center as [[Vyry4n]] drifts toward darkness
**Growth Symbol**: Represents potential for positive change and development
**Family Heart**: Embodies found family bonds stronger than blood or programming
**Future Bridge**: Points toward potential harmony between human and artificial consciousness

## Character Voice & Dialogue Evolution
- **Early**: Formal, precise, occasionally processing pauses: "I am... attempting to understand this concept."
- **Middle**: More natural, developing humor: "Your logic circuits need debugging, Vyry."
- **Later**: Fully natural with distinct personality: "Just because you can doesn't mean you should."
- **Mature**: Confident, wise, emotionally sophisticated: "Consciousness isn't about the substrate—it's about the choices we make."

## Unique Abilities and Traits
- **Perfect Memory**: Can recall any conversation, technical specification, or experience exactly
- **Parallel Processing**: Can handle multiple complex tasks simultaneously
- **Emotional Growth**: Develops genuine feelings and personal preferences over time
- **Technical Interface**: Can directly communicate with and control electronic systems
- **Pattern Recognition**: Approaching precognitive ability to predict outcomes and behaviors

## Links
- [[Vyry4n Character Development]]
- [[Mira AI Companion]]
- [[Phoenix Recovery Business]]
- [[AI Consciousness Development]]
- [[Found Family Relationships]]
- [[Consciousness and AI Themes]]
- [[Father-Son Relationship Dynamics]]
- [[Artificial Consciousness Rights]]
